...
if (cli_arg_present $args "help") {
    ...
    Write-Output '  .\provision.ps1 show_my_ip_address   # get your IP address, manually add it to provision-config.json'
...
$pg_extensions         = $configData.pg_extensions
$my_ip_address         = $configData.my_ip_address
$az_verbose_flag       = $configData.az_verbose_flag

Write-Output 'My IP Address ...'
...
Write-Host "  pg_extensions:        $pg_extensions"
Write-Host "  my_ip_address:        $my_ip_address"
Write-Host "  az_verbose_flag:      $az_verbose_flag"
Write-Host ""

if (cli_arg_present $args "show_my_ip_address") {
    Write-Output 'show_my_ip_address ...'
    (Invoke-WebRequest -uri "http://ifconfig.me/ip").Content
}
...
if (cli_arg_present $args "provision") {
    ...
    az postgres flexible-server create `
        ...
        --public-access $my_ip_address $az_verbose_flag > .\tmp\az_pg_flex_create.json
    ...
    az postgres flexible-server parameter set `
        ...
        --value $pg_extensions $az_verbose_flag > .\tmp\az_pg_flex_enable_extensions.json
